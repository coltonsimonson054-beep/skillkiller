...
src/main/java/com/smp/skillkiller/
  SkillKillerPlugin.java      - startup/shutdown, wiring, scheduled tasks
  Skill.java                  - the skill enum
  PlayerSkills.java           - one player's levels
  DataStore.java              - load/save playerdata.yml
  SkillManager.java           - config-driven effect application + level changes
  listeners/
    DeathListener.java        - the kill mechanic
    PlayerLifecycleListener.java - join/respawn/milk handling
  commands/
    SkillsCommand.java        - /skills
    SkillsAdminCommand.java   - /skillsadmin
src/main/resources/
  plugin.yml
  config.yml
```
